(() => {
  ...
  const update_intro = () => {
    ...
    scroll_fade_products.forEach((scroll_fade_product) => {
      const product_bounds = scroll_fade_product.getBoundingClientRect();
      const product_top = product_bounds.top;
      ...
      const enter_progress = ease_in_out(
        clamp((fade_start - product_top) / (fade_start - fade_end))
      );
      const exit_distance = Math.min(product_bounds.height, viewport_height * 0.35);
      const exit_progress = ease_in_out(clamp(-product_top / exit_distance));
      const opacity = Math.min(enter_progress, 1 - exit_progress);
      const offset = 18 * (1 - enter_progress) - 18 * exit_progress;

      scroll_fade_product.style.opacity = opacity.toFixed(3);
  ...
  window.addEventListener("pageshow", update_intro);
