(() => {
  ...
  const set_item_open_state = (faq_item, is_open) => {
    ...
    faq_item.answer.style.maxHeight = is_open
      ? (mobile_faq_query.matches ? "none" : `${faq_item.answer.scrollHeight}px`)
      : "0px";
  };

  const close_other_items = (active_item) => {
    faq_items.forEach((faq_item) => {
      if (faq_item !== active_item && faq_item.item.classList.contains("is-open")) {
  ...
  const sync_faq_heights = () => {
    faq_items.filter(({ item }) => item.classList.contains("is-open")).forEach((faq_item) => {
      set_item_open_state(faq_item, true);
    });
  };
  ...
  mobile_faq_query.addEventListener("change", schedule_faq_sync);
  document.fonts?.ready.then(schedule_faq_sync);
  document.fonts?.addEventListener("loadingdone", schedule_faq_sync);
})();
